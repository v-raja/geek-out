...
//                .animation(.default)
                ...
                Spacer()
                // MARK: - FOOTER
                    FooterView(showBookingAlert: self.$showAlert, altCardView: self.$altCardView)
...
//        .onAppear(perform: getUser)
            ...
            .onAppear(perform: getUser)
//        .blur(radius: self.showProfile ? 10 : 0)
            ZStack {
                ModalProfileView(showProfileView: self.$showProfile)
            ...
            .offset(x:0, y: self.showProfile ? 0 : UIApplication.shared.windows.filter {$0.isKeyWindow}.first?.frame.height ?? 0)
